...
function newsFeed() {
  ...
  var newsList = newsFeed.slice((store.currentPage - 1) * 10, store.currentPage * 10).map(function (news) {
    var id = news.id,
      title = news.title,
  ...
  var prevPage = String(store.currentPage > 1 ? store.currentPage - 1 : 1);
  var nextPage = String(store.currentPage < totalPages ? store.currentPage + 1 : totalPages);
...
function newsDetail() {
  ...
  var feedToUpdate = store.feeds.filter(function (feed) {
...
function makeComment(comments) {
  var commentString = comments.map(function (comment) {
    var level = comment.level,
      user = comment.user,
      time_ago = comment.time_ago,
      content = comment.content,
      comments = comment.comments;
    return "\n      <div style=\"padding-left: ".concat(level * 40, "px;\" class=\"mt-4\">\n        <div class=\"text-gray-400\">\n          <i class=\"fa fa-sort-up mr-2\"></i>\n          <strong>").concat(user, "</strong> ").concat(time_ago, "\n        </div>\n        <p class=\"text-gray-700\">").concat(content, "</p>\n      </div>\n      ").concat(comments.length > 0 ? makeComment(comments) : "", "\n    ");
  }).join("");
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "2102" + '/');
